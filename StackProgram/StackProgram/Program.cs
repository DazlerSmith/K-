...
namespace ArrayStackProgram
{
	class Program
	{
		...
		{
            // add your code here
            string DataItem;   // could declare outside but kept it here
            if (StackPointer == MaxStackSize)
            {
                Console.WriteLine("The stack is currently full");
                Console.ReadLine();
            }
            else
            {
                Console.WriteLine("What is the data name you would like to push");
                DataItem = Console.ReadLine();
                StackPointer = StackPointer + 1;
                TheStack[StackPointer] = DataItem;
            }
		...
		public static void PopNameFromStack(ref string[] TheStack, ref int StackPointer, int MaxStackSize)
		{
            ...
            {
                DataItem = TheStack[StackPointer];
                StackPointer = StackPointer - 1;
                Console.WriteLine(" The data item you just popped was " + DataItem);
                Console.ReadLine();   
